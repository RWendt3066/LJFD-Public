...
<head>
    ...
    // short variables for forms data
        ...
        $permitee =     'Permitee Information:\r\n'.
                        $firstname.' '.$lastname.'\r\n'.
                        (strlen($address2) >= 1 ? $address1.'\r\n'.$address2 : $address1).'\r\n'.
                        $city.', '.$state.' '.$zip.'\r\n\r\n'.
                        'Phone: '.$full_phone.'\r\n'.
                        'Email: '.$email.'\r\n\r\n';

        // eMail Burn Description Information 
        $description =  'Description of Burn:\r\n';
                if ($burn_type == 'Piled')
                {
                    $description .= $pileqty.' piles will be burned with an approximate size of '.$pileSize.' feet.\r\n'.'Material to be burned consists of:\r\n';
                    if (isset($pile_trees)) {$description .= '   - '.$pile_trees.'\r\n';};
                    if (isset($pile_brush)) {$description .= '   - '.$pile_brush.'\r\n';};
                    if (isset($pile_other)) {$description .= '   - '.$other_material.'\r\n';};
                    $description .= '\r\n';
                };
                if ($burn_type == 'Running Fire')
                {
                    $description .= 'This will be a Running Fire consisting of the following material:\r\n';
                    if (isset($running_trees)) {$description .= '   - '.$running_trees.'\r\n';};
                    if (isset($running_brush)) {$description .= '   - '.$running_brush.'\r\n';};
                    if (isset($running_grass)) {$description .= '   - '.$running_grass.'\r\n';};
                    $description .= 'The size of burn will be limited to less than 1 acre (approx. 200 ft. x 200 ft.).\r\n\r\n';
                };
                $description .= 'Date of Burn: '.$start_date.' to '.$end_date.'\r\n';
...
//        echo    'var toAddress = "rwendtpmp@gmail.com";';
        ...
        echo    'var mailContent = "'.$permitee.$description.'";';
